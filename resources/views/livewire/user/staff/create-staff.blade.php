<div>
    <div class="p-8 bg-gray-900">
        <div>
            <h1 class="text-2xl text-center text-gray-200 font-semibold">Create a new Staff</h1>
            <form wire:submit.prevent="submit" class="mt-5 text-gray-200">
                ...
                <div>
                    <select wire:model.debounce.500ms="role" class="block w-full bg-gray-800 focus:ring-0 focus:border-gray-200 rounded-md mt-10" required>
                        <option value="" selected="selected" disabled>~ Select Staff Role ~</option>
                        @foreach($roles as $role_name)
                            <option value="{{ $role_name }}" >{{ ucfirst($role_name) }}</option>
                        @endforeach 
                    </select>
                    @error('role') <span class="text-red-600 font-bold">{{ $message }}</span> @enderror
                </div>
                <div class="flex flex-col mt-10">
                    ...
                    >
                        <input 
                            ...
                            class="block w-full bg-gray-800 focus:ring-0 focus:border-gray-200 rounded-md"
                    ...
                    @error('password') <span class="text-red-600 font-bold">{{ $message }}</span> @enderror
                </div>
                <div class="flex flex-col mt-10">
                    <div 
                        x-data="{ show: false }"
                    ...
                    @error('confirm_password') <span class="text-red-600 font-bold">{{ $message }}</span> @enderror
                </div>
